...
public class Pdsp1007Mgr {
	 ...
	 * @throws Exception
	 */
	public void saveTdspMnPop(ParameterMap parameterMap) throws Exception {
		// 이미지 파일명 생성 구분자
...
//		}
		...
		tdspMnPop.setSt_dttm(parameterMap.getValue("sch_st_dt"));
		tdspMnPop.setEd_dttm(parameterMap.getValue("sch_ed_dt"));
		...
		tdspMnPop.setFrnt_yn("Y");
		tdspMnPop.setPop_kd_cd(parameterMap.getValue("pop_kd_cd"));
		tdspMnPop.setPop_url(parameterMap.getValue("pop_url"));
		
		if(!"".equals(parameterMap.getValue("srt_sqn"))){
			tdspMnPop.setSrt_sqn(Integer.parseInt(parameterMap.getValue("srt_sqn")));
		}

		// 팝업 정보 저장
		if("Y".equals(parameterMap.getValue("updateFlag"))) {
			tdspMnPop.setMn_pop_seq(Integer.parseInt(parameterMap.getValue("mn_pop_seq")));
