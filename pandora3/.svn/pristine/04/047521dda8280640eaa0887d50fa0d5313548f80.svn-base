...
//		console.log($("#module_srl option:selected").attr("module_type"));
    ...
    $("#frm_file").submit(function(e) {
    	...
    	formData.append("document_srl", _param2);
    	formData.append("module_srl", _param);
    	var files = $("#files")[0].files;
    	for(var i = 0; i < files.length; i++) formData.append("files", files[i]);
        formData.append("fileChkArr", fileChkArr);
        $.ajax({
            url : _context + '/module/board/insertBoardFileInfo.do',
            type : 'POST',
            data : formData,
	...
	$("#btnFileAdd").click(function() {
        ...
        $("input[name=fileChk]:checked").each(function () {
            ...
            board_editor.instances['content'].insertHtml(innerHtml);
        });
	});
    ...
    $("#btnFileDelete").click(function() {
        ...
        $("input[name=fileChk]:checked").each(function () {
        	var formData = new FormData();
        	var file_srl = $(this).val();
            formData.append("file_srl", file_srl);
            var fileChkId = $(this).attr("id");
        	$.ajax({
                url : _context + '/module/board/deleteBoardFileInfo.do',
                type : 'POST',
                data : formData,
                mimeType :"multipart/form-data",
                contentType : false,
...
//                    	console.log("JS  filesTotSize:::"+filesTotSize);
//                    	console.log("JS  size:::"+size);
                        $("#fileCount").html("<em>" + fileCount + "</em>개 (" + formatBytes(filesTotSize, 2) + " / 21.00 MB)");
                        if(0 == fileCount) $("#fileList").hide();
//                    	console.log("["+ file_srl +"] 파일 삭제 완료.");
                        return;
                    }else {
                        alert("정상적으로 실행되지 않았습니다. 잠시후 다시 시도하세요.")
                        return;
...
function uploadExcel(fileId, grdId, errExclTitle, exclHeader, validJsonStr) {
	...
	$.ajax({
        url         : _context + '/main/newUploadExcel',
        type        : 'POST',
        data        : formData,
        mimeType    : "multipart/form-data",
        contentType : false,
