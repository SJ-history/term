...
//법인코드 발번
function getCmpnyCode(){
    $.ajax({
        url:_context + "/pcmp/getCmpnyCode",
        type:'POST',
        data: "",
        ...
        success:function(data) {
            ...
            if(_boolean_success == json.RESULT) {
            	cmpny_cd = json.cmpnyCodeMap.cmpny_cd;
            	$("#cmpny_cd").val(cmpny_cd);
				console.log("발번완료");
            }
        },
...
//사업자등록번호 중복검사
function chkDblBizRegNo(){
    ajax({
        url: "/pcmp/checkDblBizRegNo",
        data:{biz_reg_no_val : $("#biz_reg_no_val").val()},
        success:function(data) {
            if(_boolean_success == data.RESULT) {
                var json = data.cmpnyMap;
                if(isNotEmpty(json) && isNotEmpty(json.biz_reg_no_val)){
     				dblchkflag = false;
...
function saveFormData(){
    ...
    if(chg_flag == "insert"){
        formData.append("use_cntrt_dy", "20200525" /* $("#use_cntrt_dy").val() */ );
        formData.append("fee_typ_cd", "AAAA" /* $("#fee_typ_cd").val() */ );
        formData.append("adrs_gbcd", "GBCD1" /* $("#adrs_gbcd").val() */ );
    }
...
//          selecturl   : '/pcmp/getPcmp1001SysList',	    // 그리드 조회 URL
    ...
    use_sys_grid.init();  
 	...
 	// 사업자등록번호 중복확인
 	$("#chk_dbl_biz_btn").click(function(){	    
 		chkDblBizRegNo();
 	});
	...
	<div class="frameWrap">
		<div class="subCon">
			...
			<div class="subConIn">
					...
					<table class="tblType1">   
						...
						<tbody>
					        <tr>
					          ...
					          <td>
					            <span class="txt_pw">
					            	<input type="text" id="biz_reg_no_val" name="biz_reg_no_val" class="w80" maxlength="10"/> 
					            </span>
					        ...
					        <tr>
					          <th><label for="c_cntrt_sta_dy" class="vv necessary">계약기간</label></th>
								<td class="typeCal" colspan="2">
					            	...
					            	<div id="c_cntrt_sta_dy_chk" class="regex_chk"></div>
					            	<div id="c_cntrt_end_dy_chk" class="regex_chk"></div>											
								</td>
					          <td colspan="2"></td>
					        </tr>
					        <tr>
				...
				<div class="btnWrap">
					<button type="button" class="btn_common btn_gray" id="btn_save">저장</button>
				</div>				
			</div>
		</div>
	</div>
