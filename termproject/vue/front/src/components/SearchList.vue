          ...
          <v-card-text class="" tile outlined>
            <div v-if="classList.length != 0">
              <div class="text-right">
                <v-btn @click="order" color="white">
                  최신순<span class="material-icons" id="or">arrow_downward</span>
                </v-btn>
              </div>
              <v-divider></v-divider>
            ...
            <div v-if="classList.length == 0" style="padding-top:50px">
              <NoData />
            </div>
          </v-card-text>
        </v-col>
...
export default {
  ...
  watch: {
    query: function() {
      ...
      this.$axios.post("/agency/searchList", params).then(res => {
  ...
  methods: {
    ...
    favorite(content_no) {
      if (this.$session.has("userInfo")) {
        const params = new URLSearchParams();
        params.append("user_no", this.$session.get("userInfo"));
